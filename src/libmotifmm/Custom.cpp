...
void Custom::resizeCallback(Widget widget, XtPointer clientData, XtPointer callData)
{
  Custom* custom = (Custom*)clientData;

  custom->onResize(widget, clientData, callData);
}
...
Custom::Custom(std::string name, Panel* panel)
{
  ...
  XtAddCallback(widget, XmNresizeCallback, resizeCallback, this);
...
Custom::Custom(std::string name, ScrolledPanel* panel)
{
  widget = XmCreateDrawingArea(panel->getWidget(), (char*)name.c_str(), NULL, 0);
  //XtAddCallback(widget, XmNactivateCallback, activateCallback, this);
  XtAddEventHandler(widget, ButtonPressMask, FALSE, armCallback, this);
...
void Custom::onResize(Widget widget, XtPointer clientData, XtPointer callData)
{
